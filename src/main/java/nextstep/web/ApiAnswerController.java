...
public class ApiAnswerController {
    ...
    public ResponseEntity<Void> create(@LoginUser User user, @PathVariable long id, @Valid @RequestBody String answer) {
        Answer newAnswer = qnaService.addAnswer(user, id, answer);

        HttpHeaders headers = new HttpHeaders();
        headers.setLocation(URI.create("/api/answers/" + newAnswer.getId()));
    ...
    @DeleteMapping("{id}")
    public Answer delete(@LoginUser User user, @PathVariable long id) throws CannotDeleteException, UnAuthenticationException {
